...
## using hydra
```console
reddevil@reddevil:~/Documents/htb/boxes/admirer$ hydra -L user.txt -P pass.txt ssh://10.10.10.187
...
And looking at the results, we do not get any hit.

# Back to Port 80
Now that we have discovered few folders, lets check them out on the webserver.
```console
reddevil@reddevil:~/Documents/htb/boxes/admirer/ftp$ curl 10.10.10.187/w4ld0s_s3cr3t_d1r
<!DOCTYPE HTML PUBLIC "-//IETF//DTD HTML 2.0//EN">
<html><head>
...
```
We get a 404 error which means that the directory was no longer in the server, which means that the backup on ftp is a old backup and there might be some new things on the server.

### Checking if utility-scripts exists
```console
reddevil@reddevil:~/Documents/htb/boxes/admirer/ftp$ curl 10.10.10.187/utility-scripts/
<!DOCTYPE HTML PUBLIC "-//IETF//DTD HTML 2.0//EN">
<html><head>
<title>403 Forbidden</title>
</head><body>
...
```
This directory does exist but we get a 403 response.
As there was no _w4ld0s_s3cr3t_d1r_, so I thought there might be few changes inside _utility\_scripts_ too. So I ran wfuzz to find out if there are any new php files. But the thing is that my usual go to file for dirbusting is **directory-list-2.3-medium.txt** but it turned out the needed entry was not on that list. So one of my friend gave me a hint to use "big" wordlist. So I ran wfuzz with **big.txt**. 
```console
reddevil@reddevil:~/Documents/htb/boxes/admirer/ftp/utility-scripts$ wfuzz -w /usr/share/wordlists/dirb/big.txt --hc 404 http://10.10.10.187/utility-scripts/FUZZ.php
********************************************************
...
I looked around and found a [post](https://www.foregenix.com/blog/serious-vulnerability-discovered-in-adminer-tool) explaining that this version of Adminer is vulnerable and we could pull the local files of the servers.
...
```
We got a new password for waldo in this file.
